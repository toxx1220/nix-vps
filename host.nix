...
let
  ...
  htmlGeneratorSecret = {
    owner = config.users.users.html-generator.name;
    group = config.users.groups.html-generator.name;
    mode = "0400";
  };
...
in
{
  ...
  sops = {
    ...
    secrets = {
      ...
      impressum-email = htmlGeneratorSecret;
      impressum-phone = htmlGeneratorSecret;
  ...
  systemd.services.html-generator = {
    ...
    serviceConfig = {
      Type = "oneshot";
      RuntimeDirectory = baseNameOf webRoot;
      ...
      Environment = [
        "EMAIL_FILE=${config.sops.secrets.impressum-email.path}"
        "PHONE_FILE=${config.sops.secrets.impressum-phone.path}"
        "NAME_FILE=${config.sops.secrets.impressum-name.path}"
      ];
      ExecStart = "${pkgs.writers.writeBash "generate-html-placeholders" { } (
        let
          script = pkgs.writers.writeRust "generate-html-placeholders.rs" { } (
            builtins.readFile ./scripts/generate-html-placeholders.rs
          );
        in
        ''
          set -eu
          ...
          export TEMPLATE_FILE=${./static/datenschutz.template.html}
          export OUTPUT_FILE=${webRoot}/datenschutz.html
          ${script} \
            EMAIL_FILE="$EMAIL_FILE" \
            PHONE_FILE="$PHONE_FILE" \
            NAME_FILE="$NAME_FILE"
